...
export const getServiceById = async (req, res) => {
  try {
    const { id } = req.params;
    const service = await Service.findById(id).populate("artistProfileId");
    if (!service) {
...
export const getAllServicesByUserId = async (req, res) => {
  try {
    ...
    const services = await Service.find({ artistId: artist._id }).populate("artistId");
...
export const updateService = async (req, res) => {
  try {
    ...
    const service = await Service.findByIdAndUpdate(id, updates, { new: true }).populate("artistId");
    if (!service) {
      return res.status(404).json({ success: false, message: "Service not found" });
    }
